...
if [ -f /var/workdir/tls/service-ca.crt/service-ca.crt ]; then
    keytool -import -alias jbs-cache-certificate -keystore "$FILE" -file /var/workdir/tls/service-ca.crt/service-ca.crt -storepass changeit -noprompt
fi
...
mkdir -p /var/workdir/source/logs /var/workdir/source/packages /var/workdir/source/build-info
  ...
  <profiles>
    <profile>
      ...
      <repositories>
        <repository>
          <id>artifacts</id>
          <url>file:///var/workdir/source/artifacts</url>
          <releases>
...
eval "ant $@" | tee /var/workdir/source/logs/ant.log
